...
### EnableDefaultInventory
This config property is auto-set based on `Config.CustomInventory`. When `Config.CustomInventory` is `false`, the default inventory (opened with **F2**) is enabled. Do not change this manually.
...
<Callout type="warning">
  **Deprecated** - Use `Config.CommandPermissions` as the single source of truth for command ACLs. Kept temporarily for backwards compatibility with external resources that still read `Config.AdminGroups`. Will be removed in a future version.
</Callout>
...
### CommandPermissions
With this config property you can control which ACE groups (or `user`) are allowed to use each admin command. This is the single source of truth for command ACLs.

Each command maps to a list of allowed ACE groups. The special value `"user"` makes the command available to all players.
...
### DefaultJobDuty
With this config property you can specify the default duty status a player gets when changing jobs. Set to `true` (on duty) or `false` (off duty).

### OffDutyPaycheckMultiplier
With this config property you can specify the multiplier applied to an off-duty player's paycheck. `0.5` means the player receives 50% of the on-duty paycheck.

**Example**
```lua
Config.OffDutyPaycheckMultiplier = 0.5 -- 50% of the on duty paycheck
```
...
### SaveDeathStatus
With this config property you can specify if the death status of a player should be saved. Set to `true` to save it (player keeps their death state when disconnecting) and `false` to disable.

### LocaleFallback
When a key is missing from the active locale, this config option controls whether the English string is used as a fallback instead of returning a placeholder. This is controlled by the `esx:localeFallback` convar.

- Default: `true` (fall back to English).
- Set to `"false"` to keep the placeholder (useful when authoring a new translation).

**Example**
server.cfg
```lua
setr esx:localeFallback "false"
```
