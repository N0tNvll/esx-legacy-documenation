...
Function triggered when the confirm button is pressed, returns `data` and `menu`, `data.value` is used to get the current value inserted.
...
The dialog menu opens centered with a single text input field and confirm/cancel buttons.
